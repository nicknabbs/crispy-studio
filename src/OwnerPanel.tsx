...
export function OwnerPanel({
  ...
  signedIn, isOwnerAccount, ownerEmail, ownerUserId,
}: OwnerPanelProps) {
        ...
        >
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            <Section
              ...
              subtitle={isOwnerAccount ? '✓ Owner verified' : signedIn ? 'Sign-in detected, ownership not granted' : 'Sign in to enable'}
            >
              {!signedIn && (
                <div className="bg-black/30 rounded p-3 border border-fuchsia-400/30 text-sm text-fuchsia-100">
                  <p className="mb-1">Sign in first using the 🔑 button in the top-right with the email you want as the owner account.</p>
                  <p className="text-xs text-fuchsia-200/70">Once signed in, this section will show the exact one-line SQL to copy into your Supabase SQL editor.</p>
                </div>
              )}
              {signedIn && !isOwnerAccount && ownerUserId && (
                <div className="bg-black/30 rounded p-3 border border-fuchsia-400/30 text-sm text-fuchsia-100">
                  <p className="mb-1">You're signed in as <span className="font-bold text-white">{ownerEmail}</span>, but this account isn't an owner yet.</p>
                  <p className="mb-2">Run this in your Supabase SQL editor (one-time):</p>
